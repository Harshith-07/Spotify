...
function SearchResults({ playlists, songs, artists, setView, setGlobalPlaylistId, setGlobalCurrentSongId, setGlobalIsTrackPlaying, setGlobalArtistId }) {
    ...
    return (
        <div className='flex flex-col gap-8 px-8 h-screen overflow-y-scroll scrollbar-hide text-white'>
            <div className='grid grid-cols-2'>
                <div className='space-y-4'>
                    <h2 className='text-xl font-bold text-white'>Top Results</h2>
                    <div className='h-64 pr-8'>
                        <div onClick={()=>selectPlaylist(playlists[0])} className='cursor-pointer relative group h-64 w-full bg-neutral-800 hover:bg-neutral-600 p-4 flex flex-col gap-6 rounded-md transition duration-500'>
                        <div className='absolute opacity-0 group-hover:opacity-100 transition-all ease-in-out duration-500 shadow-2xl shadow-neutral-900 h-12 w-12 flex items-center justify-center rounded-full bg-green-500 bottom-6 group-hover:bottom-8 right-8'>
                            <PlayIcon className='h-6 w-6 text-black' />
                        </div>
                        {playlists && <>
                            <img className='h-28 w-28 rounded truncate' src={playlists[0].images[0].url}/>
                ...
                <div className='space-y-4'>
                    ...
                    <div className='flex flex-col'>
                        {songs?.slice(0,4).map((song)=>(
                            <div key={song.id} onClick={()=>playSong(song)} className='cursor-default w-full h-16 px-4 rounded-md flex items-center gap-4 hover:bg-neutral-700'>
                ...
                <div className='flex flex-wrap gap-4'>
                    {artists?.slice(0,5).map((artist)=>{
                        return <div onClick={()=>selectArtist(artist)} key={artist.id} className='cursor-pointer relative group w-56 mb-2 bg-neutral-800 hover:bg-neutral-600 rounded-md p-4'>
                            <div className='absolute opacity-0 group-hover:opacity-100 transition-all ease-in-out duration-200 shadow-2xl shadow-neutral-900 h-12 w-12 flex items-center justify-center rounded-full bg-green-500 top-[156px] group-hover:top-[148px] right-6'>
                            <PlayIcon className='h-6 w-6 text-black' />
                        </div>
