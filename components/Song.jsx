...
function Song({ sno, track, setGlobalCurrentSongId, setGlobalIsTrackPlaying, setView, setGlobalArtistId }) {
    ...
    return (
        <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className='grid grid-cols-2 text-neutral-400 text-sm py-4 px-5 hover:bg-white hover:bg-opacity-10 rounded-lg cursor-default'>
            <div className='flex items-center space-x-4'>
                {hover ? <PlayIcon onClick={async () => await playSong(track)} className='h-5 w-5' /> : <p className='w-5'>{sno + 1}</p>}
                ...
                <div>
                    ...
                    <p className='w-36 truncate'>{track.artists.map((artist, i) => (
                        <>
